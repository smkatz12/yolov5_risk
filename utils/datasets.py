...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            img, labels, risks = self.load_mosaic(index)
        ...
        # print("slice shape: ", labels_out[:, 1:].shape)
        if nl:
        ...
        return torch.from_numpy(img), labels_out, self.im_files[index], shapes, torch.from_numpy(risks.squeeze())
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            # Labels
            lbls, segments = self.labels[index].copy(), self.segments[index].copy()
            risks = lbls[:, 5:]
            labels = lbls[:, :5]
            if labels.size:
    ...
    def collate_fn(batch):
        im, label, path, shapes, risks = zip(*batch)  # transposed
        for i, lb in enumerate(label):
            lb[:, 0] = i  # add target image index for build_targets()
        return torch.stack(im, 0), torch.cat(label, 0), path, shapes, risks
